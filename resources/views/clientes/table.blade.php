				...
				<tbody>
					@foreach($cliente as $list)
						<tr>
							...
							<td class="text-center">{{ substr($list->email, 0, 8) }} {{ strlen($list->email) > 8 ? '...': '' }}</td>
							<td class="text-center">{{ substr($list->grupos->nombre_grupo, 0, 10) }} {{ strlen($list->nombre_grupo) > 10 ? '...': '' }}</td>
