...
/**
     ...
     */
	//En esta funcion index se obtiene los query del grupo o cliente
    public function index(Request $request)
    {
    	//obtengo el texto seleccionado o ingresado
    	$query = $request->get('query');
		if($request->ajax() && $query != '')
	    {// si es una peticion ajax y no esta vacio
		  $output = '';
		  
		  $select = $request->get('select');
		  if($query != '')
		  {		//select para el cliente, donde se selecciona los grupos
		       	if ($select=="select"){
			     	$data = Cliente::orderBy('id','DESC')
				    ->where('grupo_id', 'like', '%'.$query.'%')
				    ->orderBy('id', 'desc')
				    ->get();
				//si esta vacio es por que es un cliente    
			    }elseif($select==""){
			     	$data = Cliente::orderBy('id','DESC')
			         ...
			         ->get();
			    }else{//de lo contrario busca el grupo
	      ...
	      {//de los contrario busca en el cliente ordenado en decreciente
		       $data = DB::table('clientes')
		         ->orderBy('id', 'desc')
		         ->get();
	      }
	      $total_row = $data->count();
	      if($total_row > 0)//si hay filas muestra
	      {
	       foreach($data as $row)
	       {
			    if ($select!="grupos"){ // si no es grupo entonces devuelve la tabla cliente   
			        //cortamos las cadenas de ser mas de 7 caracteres y agregamos puntos suspensivos
			        if (strlen($row->nombre) > 7) {
			        	$nombre=substr($row->nombre, 0, 7).' ...';
			        }else{
			        	$nombre=$row->nombre;
			        }
			        if (strlen($row->apellido) > 7) {
			        	$apellido=substr($row->apellido, 0, 7).' ...';
			        }else{
			        	$apellido=$row->apellido;
			        }
			        if (strlen($row->email) > 7) {
			        	$email=substr($row->email, 0, 7).' ...';
			        }else{
			        	$email=$row->email;
			        }
			        if (strlen($row->grupos->nombre_grupo) > 10) {
			        	$grupos=substr($row->grupos->nombre_grupo, 0, 10).' ...';
			        }else{
			        	$grupos=$row->grupos->nombre_grupo;
			        }
			        $output .= '
			        <tr>
		...
		//codificacion de json para devolver los valores
